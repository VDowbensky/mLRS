...
void tTxDisp_Init(void)
{
	...
	{
        if (setup_param_is_tx(param_idx)) tx_list.add(param_idx); 
		else if (setup_param_is_rx(param_idx)) 
		{
        //    rx_list.add(param_idx);
        } 
		else if (param_get_allowed_mask(param_idx)) common_list.add(param_idx); // to drop Ortho if not available
    }
    idx_first = 0;
    idx_focused = 0;
    idx_max = 0;
...
tTxDisp_tParamList* tTxDisp_current_list(void)
{
    switch (page) 
        case PAGE_COMMON: 
		return &common_list;
...
void tTxDisp_Tick_ms(void)
{
    ...
    // keys fast scrolling
    if (!keys_state) keys_pressed_tmo = 0;
    uint8_t fast_scroll_mask = (1 << KEY_DOWN) | (1 << KEY_UP);
	
    if (idx_focused_in_edit) fast_scroll_mask = (1 << KEY_LEFT) | (1 << KEY_RIGHT);
    if (keys_has_been_pressed & fast_scroll_mask) keys_pressed_tmo = SYSTICK_DELAY_MS(750);
    if (keys_pressed_tmo) 
	{
        keys_pressed_tmo--;
    ...
    // store
    if (page == PAGE_NOTIFY_STORE) return;
    ...
    if (setupmetadata_rx_available_last != SetupMetaData.rx_available) page_modified = true; // redraws, and avoids calling gdisp_update() multiple time
	...
	else if (key_has_been_pressed(KEY_CENTER)) 
	{
		...
		else if (page == PAGE_ACTIONS) run_action();
    }

	}
	else if (edit_setting()) 
	{
    // edit parameter
/*
...
void tTxDisp_draw_options(tParamList* list)
{
    ...
    for (uint8_t idx = 0; idx < list->num; idx++) 
	{
        ...
        if (setup_param_is_tx(param_idx) || setup_param_is_rx(param_idx)) strcpy(s, SetupParameter[param_idx].name + 3); // +3 to remove 'Tx ', 'Rx '
		else strcpy(s, SetupParameter[param_idx].name);
        ...
        if (SetupParameter[param_idx].type == SETUP_PARAM_TYPE_STR6) {
            ...
            gdisp_puts(s);
            if ((idx == idx_focused) && idx_focused_in_edit) gdisp_drawrect_WH(x + idx_focused_pos*6, gdisp.curY - 9,  11, 13, 1);
        } 
		else 
		{
            if (idx == idx_focused && idx_focused_in_edit) gdisp_setinverted();
            if (list->allowed_num[idx] == 0) strcpy(s, "-"); // unavailable
			else 
			{
                param_get_val_formattedstr(s, param_idx, PARAM_FORMAT_DISPLAY);
                // fake some settings
...
void tTxDisp_draw_page_notify(const char* s)
{
    ...
    if (len < 9) gdisp_setcurY(37-10); gdisp_puts_XCentered(s);
	else gdisp_setcurY(37-10); gdisp_puts(s); // TODO: gets the job done, but could be smarter
    gdisp_unsetfont();
}
...
// so we draw always full
    ...
    if (strlen(s) > 5) gdisp_setcurXY(35, 6);
	else  gdisp_setcurXY(50, 6);
    gdisp_puts(s);
    gdisp_setcurX(85);
