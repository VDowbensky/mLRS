...
void tTxCli_Set(uint8_t new_line_end)
{
    ...
    switch (line_end) 
	{
		case CLI_LINE_END_CR: 
		strcpy(ret, "\r"); 
		break;
	
		case CLI_LINE_END_LF: 
		strcpy(ret, "\n"); 
		break;
		
		case CLI_LINE_END_CRLF: 
		strcpy(ret, "\r\n"); 
		break;
    }
}
...
// cmd;
bool tTxCli_is_cmd(const char* cmd)
{
...
bool tTxCli_is_cmd_set_value(const char* cmd, int32_t* value)
{
	char s[64];
	uint8_t n;
...
void tTxCli_print_config_id(void)
{
    puts("ConfigId:");
...
bool tTxCli_is_cmd_param_set(char* name, char* svalue)
{
	char s[64];
	uint8_t sep, n;
...
void tTxCli_print_param_opt_list(uint8_t idx)
{
	char s[16];
...
void tTxCli_print_param(uint8_t idx)
{
...
void tTxCli_print_param_list(uint8_t flag)
{
...
void tTxCli_print_device_version(void)
{
    ...
    if (connected()) 
	{
		...
		{ // is always true when connected, except when some link task is going on or in bind
...
void tTxCli_print_frequencies(void)
{
char s[32];
...
void tTxCli_Do(void)
{
    ...
    if (state != CLI_STATE_NORMAL) 
	{
		...
		{ 
			com->getc(); 
			state = CLI_STATE_NORMAL; 
			putsn("  streaming stats stopped"); return; 
		}
        stream();
    }
    while (com->available()) 
	{
        ...
        putsn(">");
        //-- basic commands
        if (is_cmd("h")) print_help();     
		else if (is_cmd("help")) print_help(); 
		else if (is_cmd("?")) print_help();
		
		else if (is_cmd("v")) print_device_version();
		else if (is_cmd("pl"))    
		{ 
			print_config_id(); 
			print_param_list(0); 
		...
		else if (is_cmd("pl tx")) 
		{ 
			print_config_id(); 
			print_param_list(2); 
		} 
		else if (is_cmd("pl rx")) 
		{ 
			print_config_id(); 
			print_param_list(3);
        } 
		else if (is_cmd_param_set(sname, svalue)) 
		{ // p name, p name = value
            if (!param_get_idx(&param_idx, sname)) putsn("err: invalid parameter name");
			else if (!connected() && setup_param_is_rx(param_idx)) putsn("warn: receiver not connected");
			else if (svalue[0] == '?') 
			{
                print_config_id();
			...
			else if (svalue[0] == '\0') putsn("err: no value specified");
			else if (!param_set_val_fromstr(&rx_param_changed, svalue, param_idx)) putsn("err: invalid value");
			else 
			{
                print_config_id();
                print_param(param_idx);
		...
		else if (is_cmd("reload")) 
		{
			...
			else putsn("  parameters reloaded");
        } 
		else if (is_cmd_set_value("setconfigid", &value)) 
		{ // setconfigid = value
            print_config_id();
            if (value == Config.ConfigId) putsn("  no change required");
			else 
			{
		...
		else if (is_cmd("listfreqs")) print_frequencies();
		else if (is_cmd("systemboot")) //-- System Bootloader
		{
            task_pending = CLI_TASK_BOOT;
        //-- ESP handling
#ifdef USE_ESP_WIFI_BRIDGE
        } 
		else if (is_cmd("esppt")) task_pending = CLI_TASK_ESP_PASSTHROUGH; // enter esp passthrough, can only be exited by re-powering
		else if (is_cmd("espboot")) task_pending = CLI_TASK_FLASH_ESP; // enter esp flashing, can only be exited by re-powering
		else if (is_cmd("espcli")) task_pending = CLI_TASK_ESP_CLI; // enter esp cli, can only be exited by re-powering
		{
#endif
        } 
		else putsn((!pos) ? "  empty cmd" : "  invalid cmd"); //-- invalid command
        clear();
    }
}
	
int32_t GetTaskValue(void) 
{ 
	return task_value; 
}
