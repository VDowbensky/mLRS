...
void tTxCrsf_parse_nextchar(uint8_t c)
{
    ...
    switch (state) 
	{
        ...
        frame[cnt++] = c;
        if (cnt >= len + 1) state = STATE_RECEIVE_CRSF_CRC;
        break;
    ...
    case STATE_RECEIVE_CRSF_CRC:
        ...
        if (frame[2] == CRSF_FRAME_ID_CHANNELS) channels_received = true; // frame_id 
		else if (frame[0] == CRSF_OPENTX_SYNC && frame[2] == CRSF_FRAME_ID_COMMAND && frame[5] == CRSF_COMMAND_ID && frame[6] == CRSF_COMMAND_MODEL_SELECT_ID) 
		{
            cmd_modelid_received = true;
            cmd_modelid_value = frame[7];
        } 
		else cmd_received = true; 
...
bool tTxCrsf_TelemetryUpdate(uint8_t* task, uint16_t frame_rate_ms)
{
	...
	{
        case 0: 
		*task = TXCRSF_SEND_LINK_STATISTICS; 
		return true;
		
        case 1: 
		*task = TXCRSF_SEND_LINK_STATISTICS_TX; 
		return true;
		
        case 2: 
		*task = TXCRSF_SEND_LINK_STATISTICS_RX; 
		return true;
    }
...
bool tTxCrsf_CommandReceived(uint8_t* cmd)
{
    ...
    // mBridge emulation
    if (header->address == CRSF_ADDRESS_TRANSMITTER_MODULE &&  header->frame_id == CRSF_FRAME_ID_MBRIDGE_TO_MODULE) 
	{
...
void tTxCrsf_SendLinkStatistics(tCrsfLinkStatistics* payload)
{
...
int32_t mav_battery_voltage(fmav_battery_status_t* payload)
{
	...
	{
        if (payload->voltages[i] != UINT16_MAX) voltage += payload->voltages[i]; // uint16_t mV, UINT16_MAX if not known
    }
    for (uint8_t i = 0; i < 4; i++) 
	{ // we assume this never is relevant if validcellcount = false
        if (payload->voltages_ext[i] != 0) voltage += payload->voltages_ext[i]; // uint16_t mV, 0 if not known
    }
    return voltage;
}
...
void tTxCrsf_handle_mavlink_msg_global_position_int(fmav_global_position_int_t* payload)
{
    ...
    if (vfr_hud_groundspd_mps != NAN) gps.groundspeed = CRSF_REV_U16(100.0f * vfr_hud_groundspd_mps / 3.6f);
	else gps.groundspeed = 0; 
    // take the satellites of the previous reports
    if (gps_raw_int_sat != UINT8_MAX && gps2_raw_sat != UINT8_MAX) 
	{ // we have two gps
        gps.satellites = (gps_raw_int_sat > gps2_raw_sat) ? gps_raw_int_sat : gps2_raw_sat; // take the larger
    } 
	else if (gps_raw_int_sat != UINT8_MAX)  gps.satellites = gps_raw_int_sat;
	else gps.satellites = 0; 
    // mark as updated
    gps_updated = true;
}
