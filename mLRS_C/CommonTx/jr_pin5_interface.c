...
void uart_tx_putc_totxbuf(char c)
{
	...
	{ // fifo not full //this is isr safe, works also if readpos has changed in the meanwhile
        uart_txbuf[next] = c;
...
void tPin5BridgeBase_uart_rx_callback(uint8_t c)
{
	...
	{ // we are in transmitting, should not happen! (does appear to not happen)
        state = STATE_IDLE;
        return;
    }
    if (transmit_start()) 
	{ // check if a transmission waits, put it into buf and return true to start
        pin5_tx_enable(true);
...
void tPin5BridgeBase_CheckAndRescue(void)
{
    ...
    if (state < STATE_TRANSMITING) nottransmiting_tlast_ms = tnow_ms;
	else 
	{
        if (tnow_ms - nottransmiting_tlast_ms > 20) 
		{ // we are stuck, so rescue
#ifdef TX_FRM303_F072CB
            gpio_low(IO_PB9);
#endif
#if defined TX_DIY_SXDUAL_MODULE02_G491RE || defined TX_DIY_E28DUAL_MODULE02_G491RE || defined TX_DIY_E22DUAL_MODULE02_G491RE
