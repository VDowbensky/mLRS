...
void tRxMavlink_Do(void)
{
    ...
    if (fifo_link_out.HasSpace(290)) 
	{ // we have space for a full MAVLink message, so can safely parse
        while (serial.available()) 
		{
            ...
            if (result.res == FASTMAVLINK_PARSE_RESULT_OK) 
			{
                ...
                uint16_t len;
                if (Setup.Rx.SerialLinkMode == SERIAL_LINK_MODE_MAVLINK_X) len = fmavX_msg_to_frame_buf(_buf, &msg_link_out); // X frame now in _buf
				else len = fmav_msg_to_frame_buf(_buf, &msg_link_out);
                fifo_link_out.PutBuf(_buf, len);
                bytes_parser_in = 0;
                if (msg_link_out.msgid == FASTMAVLINK_MSG_ID_COMMAND_LONG) handle_cmd_long();
    ...
    if (inject_rc_channels) 
	{ // give it priority // && serial.tx_is_empty()) // check available size!?
        inject_rc_channels = false;
...
void tRxMavlink_putc(char c)
{
    ...
    fmav_result_t result;
#ifdef USE_FEATURE_MAVLINKX
    if (Setup.Rx.SerialLinkMode == SERIAL_LINK_MODE_MAVLINK_X) fmavX_parse_and_check_to_frame_buf(&result, buf_link_in, &status_link_in, c);
	else fmav_parse_and_check_to_frame_buf(&result, buf_link_in, &status_link_in, c);
#else
    ...
    if (result.res == FASTMAVLINK_PARSE_RESULT_OK) 
	{
        ...
        switch (Config.Mode) 
		{
			...
			force_param_list = (Config.SerialBaudrate > 230400); 
			break; // 230400 bps and lower is ok for mftp
			
			case MODE_50HZ:
			case MODE_FSK_50HZ: 
			force_param_list = (Config.SerialBaudrate > 57600);
			break; // 57600 bps and lower is ok for mftp
			
			case MODE_31HZ: force_param_list = (Config.SerialBaudrate > 57600); 
			break; // 57600 bps and lower is ok for mftp
		
			case MODE_19HZ: force_param_list = (Config.SerialBaudrate > 38400); 
			break; // 38400 bps and lower is ok for mftp
        }
        if (force_param_list)
#endif
...
void tRxMavlink_send_msg_serial_out(void)
{
...
uint16_t tRxMavlink_serial_in_available(void)
{
#ifdef USE_FEATURE_MAVLINKX
...
bool tRxMavlink_handle_txbuf_ardupilot(uint32_t tnow_ms)
{
    // work out state
    bool inject_radio_status = false;
	...
	else  if ((tnow_ms - radio_status_tlast_ms) >= 100) 
	{ // limit to 10 Hz
        switch (txbuf_state) 
		{
			case TXBUF_STATE_NORMAL:
            if (serial_in_available() > 1024) 
			{ // ups, suddenly lots of traffic
            ...
            if (serial_in_available() > 1024) 
			{ // it hasn't depleted, so raise alarm high
                txbuf_state = TXBUF_STATE_BURST_HIGH;
                radio_status_tlast_ms = tnow_ms;
                inject_radio_status = true;
            } 
			else if (serial_in_available() < 384) { // quite empty, so we can go back and try normal
                txbuf_state = TXBUF_STATE_NORMAL;
                radio_status_tlast_ms = tnow_ms;
                inject_radio_status = true;
            }
            ...
            if (serial_in_available() < 1024) txbuf_state = TXBUF_STATE_BURST;// it has depleted, we can go back and try burst
            radio_status_tlast_ms = tnow_ms;
            inject_radio_status = true;
            break;
        }
    ...
    if (rate_percentage > 95) txbuf = 0; // ArduPilot:  0-19  -> +60 ms,    PX4:  0-24  -> *0.8
	else if (rate_percentage > 85) txbuf = 30; // ArduPilot: 20-49  -> +20 ms,    PX4: 25-34  -> *0.975
	else if (rate_percentage < 60) txbuf = 100; // ArduPilot: 96-100 -> -40 ms,    PX4: 51-100 -> *1.025
	else if (rate_percentage < 75) txbuf = 91; // ArduPilot: 91-95  -> -20 ms,    PX4: 51-100 -> *1.025
	else txbuf = 50; // ArduPilot: 50-90  -> no change, PX4: 35-50  -> no change
    if (txbuf_state == TXBUF_STATE_BURST_HIGH) txbuf = 0; // try to slow down as much as possible
	else if (txbuf_state == TXBUF_STATE_BURST) txbuf = 50; // cut out PARAMS but don't change stream rate
	else if ((txbuf_state == TXBUF_STATE_NORMAL) && (txbuf_state_last > TXBUF_STATE_NORMAL)) 
	{ // has changed back to NORMAL
...
if(txbuf_state==2) dbg.puts("high, "); 
else if(txbuf_state==1) dbg.puts("brst, "); 
else dbg.puts("norm, ");
dbg.puts(u8toBCD_s(txbuf));dbg.puts(", ");
if(txbuf<20) dbg.puts("+60 "); 
else if(txbuf<50) dbg.puts("+20 "); 
else if(txbuf>95) dbg.puts("-40 "); 
else if(txbuf>90) dbg.puts("-20 ");
else dbg.puts("0   ");
#endif
	...
	else bytes_link_out = 0; // reset, to restart rate measurement
...
bool tRxMavlink_handle_txbuf_method_b(uint32_t tnow_ms)
{
    // work out state
    bool inject_radio_status = false;
    ...
    switch (txbuf_state) 
	{
			...
			case TXBUF_STATE_BURST:
            if (serial_in_available() > 1400) 
			{ // still growing, so raise alarm high
                txbuf_state = TXBUF_STATE_BURST_HIGH;
                radio_status_tlast_ms = tnow_ms;
                inject_radio_status = true;
            } 
			else if (serial_in_available() < FRAME_RX_PAYLOAD_LEN*2) 
			{ // less than 2 radio messages remain, back to normal
			...
			case TXBUF_STATE_BURST_HIGH:
            if ((tnow_ms - radio_status_tlast_ms) >= 100) 
			{ // limit to 10 Hz
                if (serial_in_available() < 1400) txbuf_state = TXBUF_STATE_BURST; // it has stopped growing, we can go back and try burst
                radio_status_tlast_ms = tnow_ms;
                inject_radio_status = true;
            }
            break;
...
uint32_t t = millis32(), dt = t - t_last; t_last = t;
dbg.puts("\nMp: ");
dbg.puts(u16toBCD_s(t));
dbg.puts(" (");
dbg.puts(u16toBCD_s(dt));
dbg.puts("), ");
//dbg.puts(u16toBCD_s(stats.GetTransmitBandwidthUsage()*41));dbg.puts(", ");
dbg.puts(u16toBCD_s(bytes_link_out));
dbg.puts(", ");
dbg.puts(u16toBCD_s(serial_in_available()));
dbg.puts(", ");
dbg.puts(u8toBCD_s((rate_percentage<256)?rate_percentage:255));
dbg.puts(", ");
if(txbuf_state==1) dbg.puts("brst, "); 
else if(txbuf_state==2) dbg.puts("high, "); 
else if(txbuf_state==3) dbg.puts("recv, "); 
else dbg.puts("norm, ");
dbg.puts(u8toBCD_s(txbuf));
dbg.puts(", ");
if(txbuf<25) dbg.puts("*0.8 "); 
else if(txbuf<35) dbg.puts("*0.975 "); 
else if(txbuf>50) dbg.puts("*1.025 "); 
else dbg.puts("*1 ");
#endif
...
void tRxMavlink_generate_radio_status(void)
{
...
void tRxMavlink_generate_rc_channels_override(void)
{
...
void tRxMavlink_generate_radio_rc_channels(void)
{
...
void tRxMavlink_generate_radio_link_stats(void)
{
...
void tRxMavlink_generate_radio_link_information(void)
{
...
void tRxMavlink_generate_cmd_ack(void)
{
...
void tRxMavlink_handle_cmd_long(void)
{
#ifdef USE_FEATURE_MAVLINKX
fmav_command_long_t payload;
    ...
    if (payload.command != cmd_ack.command) cmd_ack.state = 0; // a new sequence is started, so reset
    inject_cmd_ack = true;
