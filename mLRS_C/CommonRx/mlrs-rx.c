...
//dbg.putc(payload[payload_len]);
    ...
    if (transmit_frame_type == TRANSMIT_FRAME_TYPE_NORMAL) pack_rxframe(&rxFrame, &frame_stats, payload, payload_len);
	else pack_rxcmdframe(&rxFrame, &frame_stats);
}
...
uint8_t do_receive(uint8_t antenna, bool do_clock_reset) // we receive a frame from receiver
{
    ...
    if (bind.IsInBind()) return bind.do_receive(antenna, do_clock_reset);
	...
	{ 
		FAIL_WMSG("do_receive() CHECK_ERROR_SYNCWORD"); 
...
/*            dbg.puts("\nRX: ");
    ...
    switch (link_state) 
	{
        ...
        if (connect_state >= CONNECT_STATE_SYNC) fhss.HopToNext(); // we hop only if not in listen
        sx.SetRfFrequency(fhss.GetCurrFreq());
        sx2.SetRfFrequency(fhss.GetCurrFreq());
...
//dbg.puts(" a "); dbg.puts((antenna == ANTENNA_1) ? "1 " : "2 ");
        ...
        if (!valid_frame_received) mavlink.FrameLost();
        ...
        if ((connect_state == CONNECT_STATE_LISTEN) && invalid_frame_received) link_state = LINK_STATE_RECEIVE;
        ...
        if ((connect_state >= CONNECT_STATE_SYNC) || (link_state == LINK_STATE_RECEIVE) || (link_state == LINK_STATE_TRANSMIT)) 
		{
            sx.SetToIdle();
            sx2.SetToIdle();
        }
        ...
        if (!tick_1hz_commensurate) rxstats.Update1Hz();
        rxstats.Next();
