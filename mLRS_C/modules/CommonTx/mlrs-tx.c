...
void WhileTransmit::handle_once(void)
{
    ...
    if (tnow_ms - main_tlast_ms >= 250) 
	{ // Update Main page at 4 Hz
    ...
    if (tnow_ms - draw_tlast_ms >= 40) 
	{ // effectively slows down (drawing takes time, ca 30 ms on G4, slower on other mcu)
...
RESTARTCONTROLLER:
  ...
  while (1) 
  {
    ...
    if (doSysTask) 
	{
        ...
        if (connect_tmo_cnt) connect_tmo_cnt--;
        if (connected()) DECc(led_blink, SYSTICK_DELAY_MS(500));
		else DECc(led_blink, SYSTICK_DELAY_MS(200));
...
IF_SX(
    if (irq_status) 
	{
		...
		else if (link_state == LINK_STATE_RECEIVE_WAIT) 
		{
            if (irq_status & SX_IRQ_RX_DONE) 
			{
                irq_status = 0;
		...
		{ // this should not happen
            ...
            if (irq_status & SX_IRQ_RX_DONE) FAIL_WSTATE(BLINK_RD_GR_OFF, "IRQ RX DONE FAIL", irq_status, link_state, link_rx1_status, link_rx2_status);
            if (irq_status & SX_IRQ_TX_DONE) FAIL_WSTATE(BLINK_GR_RD_OFF, "IRQ TX DONE FAIL", irq_status, link_state, link_rx1_status, link_rx2_status);
            irq_status = 0;
            link_state = LINK_STATE_IDLE;
            link_rx1_status = link_rx2_status = RX_STATUS_NONE;
            DBG_MAIN_SLIM(dbg.puts("1?");)
...
IF_SX2(
    if (irq2_status) 
	{
		...
		{ // this should not happen
            ...
            if (irq2_status & SX2_IRQ_RX_DONE) FAIL_WSTATE(BLINK_RD_GR_ON, "IRQ2 RX DONE FAIL", irq2_status, link_state, link_rx1_status, link_rx2_status);
            if (irq2_status & SX2_IRQ_TX_DONE) FAIL_WSTATE(BLINK_GR_RD_ON, "IRQ2 TX DONE FAIL", irq2_status, link_state, link_rx1_status, link_rx2_status);
            irq2_status = 0;
            link_state = LINK_STATE_IDLE;
            link_rx1_status = link_rx2_status = RX_STATUS_NONE;
            DBG_MAIN_SLIM(dbg.puts("2?");)
    ...
    if (doPreTransmit) 
	{
		...
		{ // frame received
            ...
            if (USE_ANTENNA1 && USE_ANTENNA2) antenna = rdiversity.Antenna(link_rx1_status, link_rx2_status, stats.last_rssi1, stats.last_rssi2);
			else if (USE_ANTENNA2) antenna = ANTENNA_2;
            handle_receive(antenna);
        } 
		else handle_receive_none();

        if (TRANSMIT_USE_ANTENNA1 && TRANSMIT_USE_ANTENNA2) tdiversity.DoEstimate(link_rx1_status, link_rx2_status, stats.last_rssi1, stats.last_rssi2);
		else if (TRANSMIT_USE_ANTENNA2) tdiversity.SetAntenna(ANTENNA_2);
		else tdiversity.SetAntenna(ANTENNA_1);
...
IF_MBRIDGE_OR_CRSF( // to allow crsf mbridge emulation
    ...
    if (mbridge.CommandReceived(&mbcmd)) 
	{
        switch (mbcmd) 
		{
			...
			mbridge.HandleCmd(MBRIDGE_CMD_PARAM_REQUEST_LIST); 
			break;
...
IF_CRSF(
    ...
    if (crsf.TelemetryUpdate(&crsftask, Config.frame_rate_ms)) 
	{
        switch (crsftask) 
		{
			case TXCRSF_SEND_LINK_STATISTICS: 
			crsf_send_LinkStatistics(); 
			do_cnt = 0; 
			break;
			
			case TXCRSF_SEND_LINK_STATISTICS_TX: 
			crsf_send_LinkStatisticsTx(); 
			break;
			
			case TXCRSF_SEND_LINK_STATISTICS_RX: 
			crsf_send_LinkStatisticsRx(); 
			break;
			...
			else if (connected_and_rx_setup_available() && SERIAL_LINK_MODE_IS_MAVLINK(Setup.Rx.SerialLinkMode)) 
			{
...
IF_IN(
    ...
    switch (cli_task) 
	{
		...
		case CLI_TASK_FLASH_ESP: 
		esp.EnterFlash(); 
		break;
		
		case CLI_TASK_ESP_CLI: 
		esp.EnterCli(); 
		break;
		
		case CLI_TASK_ESP_PASSTHROUGH: 
		esp.EnterPassthrough(); 
		break;
		
		case CLI_TASK_CHANGE_CONFIG_ID: 
		config_id.Change(cli.GetTaskValue()); 
		break;
