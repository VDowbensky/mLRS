...
uint8_t tRDiversity::Antenna(uint8_t link_rx1_status, uint8_t link_rx2_status, int8_t rssi1, int8_t rssi2)
{
	...
	else if (link_rx1_status == RX_STATUS_VALID) 
	{
        antenna = ANTENNA_1;
    } 
	else if (link_rx2_status == RX_STATUS_VALID) 
	{
        antenna = ANTENNA_2;
#ifdef DEVICE_IS_RECEIVER
    } 
	else if (link_rx1_status == RX_STATUS_CRC1_VALID) antenna = ANTENNA_1;
	else if (link_rx2_status == RX_STATUS_CRC1_VALID) 
	{
        antenna = ANTENNA_2;
#endif
    } 
	else if (link_rx1_status == RX_STATUS_INVALID) antenna = ANTENNA_1;
	else if (link_rx2_status == RX_STATUS_INVALID) antenna = ANTENNA_2;
	else antenna = ANTENNA_1; // this should never happen!
    return antenna;
}
...
int16_t estimator_step;
	...
	else if (invalid1_cnt) invalid1_cnt--;
    if (link_rx2_status <= RX_STATUS_INVALID) 
	{
        if (invalid2_cnt < 5) invalid2_cnt++;
    } 
	else if (invalid2_cnt) invalid2_cnt--;
    // now run the estimator
#ifdef DEVICE_IS_RECEIVER
    if ((link_rx1_status == RX_STATUS_VALID && link_rx2_status == RX_STATUS_VALID) || (link_rx1_status == RX_STATUS_CRC1_VALID && link_rx2_status == RX_STATUS_CRC1_VALID)) 
	{
#else
    if (link_rx1_status == RX_STATUS_VALID && link_rx2_status == RX_STATUS_VALID) 
	{
        ...
        if (rssi1 > rssi2) estimator_step = frame_rate_ms; // go towards antenna 1
		else if (rssi1 < rssi2) estimator_step = -frame_rate_ms; // go towards antenna 2
		else estimator_step = 0; // stay with current antenna
    } 
	else if (link_rx1_status > link_rx2_status) estimator_step = +TDIVERSITY_INVALID_STEP;// antenna 1 is better, go quickly to antenna 1
	else if (link_rx1_status < link_rx2_status) estimator_step = -TDIVERSITY_INVALID_STEP;// antenna 2 is better, go quickly to antenna 2
	else if (link_rx1_status == RX_STATUS_INVALID && link_rx2_status == RX_STATUS_INVALID) 
	{
        // go to the one which is less often invalid
        if (invalid1_cnt < invalid2_cnt) estimator_step = TDIVERSITY_RANGE; // choose antenna 1
		else if (invalid1_cnt > invalid2_cnt) estimator_step = -TDIVERSITY_RANGE; // choose antenna 2
		else 
		{
	...
	else 
	{
        ...
        if (estimator_step_last >= 0) 
		{
            ...
            if (estimator_value < TDIVERSITY_RANGE) estimator_step = frame_rate_ms; // go further towards antenna 1
			else estimator_step = -frame_rate_ms; // swap direction and go back to antenna 2
        } 
		else 
		{
            // we are moving in direction of antenna 1
            if (estimator_value > -TDIVERSITY_RANGE) estimator_step = -frame_rate_ms; // go further towards antenna 2
			else estimator_step = frame_rate_ms; // swap direction and go back to antenna 1
        }
