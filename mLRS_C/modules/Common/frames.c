...
void _pack_txframe_w_type(tTxFrame* frame, uint8_t type, tFrameStats* frame_stats, tRcData* rc, uint8_t* payload, uint8_t payload_len)
{
    ...
    // pack the payload
    for (uint8_t i = 0; i < payload_len; i++) frame->payload[i] = payload[i];
    // finalize, crc
    fmav_crc_init(&crc);
...
uint8_t check_txframe(tTxFrame* frame)
{
    ...
    if ((frame->status.frame_type != FRAME_TYPE_TX) && (frame->status.frame_type != FRAME_TYPE_TX_RX_CMD)) return CHECK_ERROR_HEADER;
...
uint8_t check_rxframe(tRxFrame* frame)
{
    ...
    if (frame->sync_word != Config.FrameSyncWord) return CHECK_ERROR_SYNCWORD;
    if ((frame->status.frame_type != FRAME_TYPE_RX) && (frame->status.frame_type != FRAME_TYPE_TX_RX_CMD)) return CHECK_ERROR_HEADER;
...
void pack_rxcmdframe_rxsetupdata(tRxFrame* frame, tFrameStats* frame_stats)
{
    ...
    for (uint8_t i = 0; i < 8; i++) rx_setupdata.Power_list[i] = (i < RFPOWER_LIST_NUM) ? rfpower_list[i].mW : INT16_MAX;
