...
void tPassThrough_handle_mavlink_msg_heartbeat(fmav_heartbeat_t* payload) // #0 -> 0x5001 AP_STATUS, 0x5007 PARAM
{
...
void tPassThrough_handle_mavlink_msg_sys_status(fmav_sys_status_t* payload) // #1 -> 0x5005 VEL_YAW
{
...
void tPassThrough_handle_mavlink_msg_gps_raw_int(fmav_gps_raw_int_t* payload) // #24 -> 0x5002 GPS STATUS
{
...
void tPassThrough_handle_mavlink_msg_raw_imu(fmav_raw_imu_t* payload) // #27 -> 0x5001 AP_STATUS
{
...
void tPassThrough_handle_mavlink_msg_attitude(fmav_attitude_t* payload) // #30 -> 0x5006 ATTITUDE_RANGE
{
    memcpy(&attitude, payload, sizeof(fmav_attitude_t));
    pt_update[ATTITUDE_RANGE_0x5006] = true;
}

void tPassThrough_handle_mavlink_msg_global_position_int(fmav_global_position_int_t* payload) // #33 -> 0x800 (2x), 0x5004 HOME
{
...
void tPassThrough_handle_mavlink_msg_mission_current(fmav_mission_current_t* payload) // #42 -> 0x5009 WAYPOINT_V1, 0x500D WAYPOINT V2
{
...
void tPassThrough_handle_mavlink_msg_nav_controller_output(fmav_nav_controller_output_t* payload) // #62 -> 0x5009 WAYPOINT_V1, 0x500D WAYPOINT V2
{
...
void tPassThrough_handle_mavlink_msg_vfr_hud(fmav_vfr_hud_t* payload) // #74 -> 0x5005 VEL_YAW, 0x5001 AP_STATUS, 0x50F2 VFR_HUD
{
...
void tPassThrough_handle_mavlink_msg_terrain_report(fmav_terrain_report_t* payload) // #136 -> 0x500B TERRAIN
{
    memcpy(&terrain_report, payload, sizeof(fmav_terrain_report_t));
    // pt_update[TERRAIN_0x500B] = true; // TODO
}

void tPassThrough_handle_mavlink_msg_battery_status(fmav_battery_status_t* payload) // #147 -> 0x5003 BATT_1, 0x5008 BATT_2
{
    if (payload->id == 0) 
	{
...
void tPassThrough_handle_mavlink_msg_fence_status(fmav_fence_status_t* payload) // #162 -> 0x5001 AP_STATUS
{
...
void tPassThrough_handle_mavlink_msg_rangefinder(fmav_rangefinder_t* payload) // #173 -> 0x5006 ATTITUDE_RANGE
{
    memcpy(&rangefinder, payload, sizeof(fmav_rangefinder_t));
    pt_update[ATTITUDE_RANGE_0x5006] = true; // TODO
}

void tPassThrough_handle_mavlink_msg_rpm(fmav_rpm_t* payload) // #226 -> 0x500A RPM
{
    memcpy(&rpm, payload, sizeof(fmav_rpm_t));
    pt_update[RPM_0x500A] = true; // TODO
}

void tPassThrough_handle_mavlink_msg_home_position(fmav_home_position_t* payload) // #242 -> 0x5004 HOME
{
...
void tPassThrough_handle_mavlink_msg_statustext(fmav_statustext_t* payload) // #253 -> 0x5000 TEXT
{
...
void tPassThrough_pt_pack32(uint32_t* value, uint32_t data, uint8_t pos, uint8_t len)
{
    uint32_t mask = 0;
...
*/
    ...
    if (!statustext_cur_inprocess) 
	{ // idle, so we can check for a new statustext
...
bool tPassThrough_get_ApStatus_0x5001(uint32_t* data)
{
    ...
    if (pt_throt <= 0) pt_throt = 0;
	else if (pt_throt >= 100) pt_throt = 63;
	else pt_throt = (pt_throt * 63  + 49) / 100;

    int32_t pt_imu_temp = 0;
	
    if (raw_imu.temperature != 0) 
	{ // 0 = invalid
...
bool tPassThrough_get_GpsStatus_0x5002(uint32_t* data)
{
    ...
    if (gps_raw_int.fix_type >= GPS_FIX_TYPE_3D_FIX) 
	{
        ...
        if (pt_altitude_MSL > 127000) pt_altitude_MSL = 127000;
		else if (pt_altitude_MSL < -127000) pt_altitude_MSL = -127000;
    }
    *data = 0;
...
bool tPassThrough_get_Home_0x5004(uint32_t* data)
{
    ...
    float direction = atan2f( sinf(lon2 - lon1) * cos_lat2,cos_lat1 * sinf(lat2) - sinf(lat1) * cos_lat2 * cosf(lon2 - lon1));
...
bool tPassThrough_get_WayPointV2_0x500D(uint32_t* data)
{
    ...
    if (passthrough_array_is_receiving) 
	{ 
		*data = pt_data[WAYPOINT_V2_0x500D]; 
		return true; 
	}
    *data = 0;
    return true;
