...
{
  public:
    void tFhssBase_Init(uint8_t fhss_num, uint32_t seed, uint8_t frequency_band, uint16_t fb_allowed_mask, uint8_t ortho, uint8_t except)
    {
    ...
    void tFhssBase_Start(void)
    {
        curr_i = 0;
    }

    // only used for statistics
    uint8_t tFhssBase_Cnt(void)
    {
        return cnt;
    }

    // only used for statistics
    uint8_t tFhssBase_CurrI(void)
    {
        return curr_i;
    }

    uint32_t tFhssBase_GetCurrFreq(void)
    {
        if (is_in_binding) {
    ...
    void tFhssBase_HopToNext(void)
    {
        curr_i++;
        if (curr_i >= cnt) curr_i = 0;
    }

    void tFhssBase_SetToBind(uint16_t frame_rate_ms = 1) // preset so it is good for transmitter
    {
        is_in_binding = true;
    ...
    bool tFhssBase_HopToNextBind(void)
    {
    ...
    uint8_t tFhssBase_GetCurrFrequencyBand(void)
    {
    ...
    uint8_t tFhssBase_ChList(uint8_t i) 
	{ 
		return ch_list[i]; 
	}
	
    uint32_t tFhssBase_FhssList(uint8_t i) 
	{ 
		return fhss_list[i]; 
	}
...
class tFhss : public tFhssBase
{
  public:
    ...
    {
        tFhssBase_Init(fhss->Num, fhss->Seed,
