...
uint16_t tFhssBase_prng(void);
void tFhssBase_generate(uint32_t seed);
void tFhssBase_generate_ortho_except(uint32_t seed, uint8_t ortho, uint8_t except);
...
void tFhssBase_generate(uint32_t seed)
{
    ...
    while (k < cnt) 
	{
        ...
        if (ch >= FREQ_LIST_LEN) ch = 0; // argh, must not happen !
        ...
        if ((config_i != FHSS_CONFIG_433_MHZ && config_i != FHSS_CONFIG_866_MHZ_IN) && (k > 0)) 
		{ // TODO: use smarter method, e.g., cnt < 2/3
    ...
    for (uint8_t k = 0; k < cnt; k++) fhss_last_rssi[k] = INT8_MIN;
}
...
void tFhssBase_generate_ortho_except(uint32_t seed, uint8_t ortho, uint8_t except)
{
    ...
    while (k < cnt) 
	{
        ...
        if (ch_eff >= freq_len) ch_eff = freq_len; // argh, must not happen !
